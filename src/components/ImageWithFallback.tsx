...
export default function ImageWithFallback({
  ...
  if (currentSrc >= sources.length) {
    ...
    return (
      <div 
        className={`bg-gradient-to-br from-primary-blue/20 to-primary-dark/20 rounded-lg flex items-center justify-center border border-primary-blue/30 ${className}`}
        style={fill ? {} : { width, height }}
      >
        <div className="text-white/70 text-center">
          <div className="text-4xl mb-2 animate-pulse">🌟</div>
          <div className="text-xs font-mono">Luminous Core</div>
          <div className="text-xs text-primary-blue/70">{baseName}</div>
        </div>
      </div>
    )
  }
